...
describe('listToTree', () => {
  ...
  it('nodeAssign', () => {
    ...
    const result = listToTree(basicMenus, { childrenField: 'childs' });

    expect(result[0]).toMatchObject(basicMenus[0]);
    expect(result[1]).toMatchObject(basicMenus[1]);
    expect(result[1].childs[0]).toMatchObject(basicMenus[2]);
    expect(result[1].childs[0].childs[0]).toMatchObject(basicMenus[3]);
    expect(result[2]).toMatchObject(basicMenus[4]);
    expect(result[2].childs[0]).toMatchObject(basicMenus[5]);
    expect(result[2].childs[1]).toMatchObject(basicMenus[6]);
  });
});